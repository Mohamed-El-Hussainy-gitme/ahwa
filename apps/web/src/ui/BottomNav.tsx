...
function buildTabs(input: {
  role: 'owner' | 'branch_manager' | 'supervisor' | 'waiter' | 'american_waiter' | 'barista' | 'shisha' | 'unassigned';
  ...
  if (role === 'owner' || role === 'branch_manager') {
    return [
      ...
      { href: '/owner', label: role === 'branch_manager' ? 'الفرع' : 'الإدارة', icon: 'crown', badge: 0 },
    ];
  }
...
export function BottomNav() {
  ...
  const role: 'owner' | 'branch_manager' | 'supervisor' | 'waiter' | 'american_waiter' | 'barista' | 'shisha' | 'unassigned' = can.owner
    ? 'owner'
    : can.branchManager
      ? 'branch_manager'
      ...
      {tabs.map((tab) => {
        ...
        return (
          <Link key={tab.href} href={tab.href} className={[
            ...
            active ? 'border-[#1e1712] bg-[#1e1712] text-white shadow-[0_12px_24px_rgba(30,23,18,0.16)]' : 'border-[#dccdbb] bg-[#fffaf4] text-[#4e4034] hover:bg-[#f6ede2]',
          ].join(' ')}>
            {tab.badge > 0 ? <span className={[
            ...
            ].join(' ')}>{tab.badge > 99 ? '99+' : tab.badge}</span> : null}
