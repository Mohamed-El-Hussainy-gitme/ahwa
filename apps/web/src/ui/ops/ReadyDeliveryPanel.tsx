...
function groupReadyItems(items: ReadyItem[]): ReadyItemGroup[] {
  const groups = new Map<string, ReadyItemGroup>();

  for (const item of items) {
    const groupKey = item.serviceSessionId || item.sessionLabel;
...
export function ReadyDeliveryPanel({
  ...
  const groupedItems = groupReadyItems(items);
        ...
        {groupedItems.length ? (
          <div className="space-y-3">
            {groupedItems.map((group, groupIndex) => (
              <div key={group.sessionId || group.sessionLabel} className={groupIndex > 0 ? 'border-t border-[#eee2d6] pt-3' : ''}>
                <div className="mb-2 text-right text-base font-black text-[#2d241d]">{group.sessionLabel}</div>
  ...
  return (
    <div className={[opsSurface, 'p-3'].join(' ')}>
      ...
      <div className="space-y-3">
        {groupedItems.map((group, groupIndex) => (
          <div key={group.sessionId || group.sessionLabel} className={groupIndex > 0 ? 'border-t border-[#eee2d6] pt-3' : ''}>
            <div className="mb-2 text-right text-lg font-black text-[#2d241d]">{group.sessionLabel}</div>
            <div className="space-y-3">
              {group.items.map((item) => {
                const quantity = Math.max(1, Math.min(selectedQty[item.orderItemId] ?? 1, item.qtyReadyForDelivery));
                const parsedNotes = parseOrderItemNotes(item.notes);
        ...
        {!groupedItems.length ? <div className={[opsDashed, 'p-3 text-sm text-[#6b5a4c]'].join(' ')}>{emptyLabel}</div> : null}
      </div>
    </div>
  );
}
