...
function groupReadyItems(items: ReadyItem[]) {
  const groups: Array<{ sessionLabel: string; items: ReadyItem[] }> = [];
  const lookup = new Map<string, { sessionLabel: string; items: ReadyItem[] }>();

  for (const item of items) {
    const key = String(item.sessionLabel ?? '').trim() || 'بدون جلسة';
    ...
    const group = { sessionLabel: key, items: [item] };
  ...
  return (
    <div className={[opsSurface, 'p-3'].join(' ')}>
      ...
      <div className="space-y-4">
        {groupedItems.map((group, groupIndex) => (
          <section key={`${group.sessionLabel}-${groupIndex}`} className={groupIndex > 0 ? 'border-t border-[#eadfce] pt-4' : ''}>
            <div className="mb-3 text-lg font-black text-[#2a2018]">{group.sessionLabel}</div>
