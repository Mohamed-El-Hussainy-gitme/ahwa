    ...
    <div className="flex min-h-dvh items-center justify-center bg-[linear-gradient(180deg,#f4efe7_0%,#eadcc8_100%)] p-4">
      <div className="w-full max-w-sm overflow-hidden rounded-[32px] border border-[#d9cabb] bg-[#fffaf4] shadow-[0_28px_72px_rgba(30,23,18,0.12)]">
        <div className="border-b border-[#eadfce] bg-[linear-gradient(180deg,#fffaf4_0%,#f5eadc_100%)] px-6 pb-5 pt-6">
          ...
          {resolvedSlug ? (
            <div className="mt-4 flex items-center justify-center gap-2 rounded-[20px] border border-[#e6d8c8] bg-[#fbf5ed] px-4 py-3 text-sm font-semibold text-[#4e4034]">
              <AppIcon name="building" className="h-4 w-4 text-[#9b6b2e]" />
              <span>القهوة:</span>
              <span className="font-black text-[#1e1712]" dir="ltr">{resolvedSlug}</span>
            </div>
          ) : null}
        </div>
          ...
          <form className="space-y-3" onSubmit={onSubmit}>
            <div>
              ...
              <div className="relative">
                <input
                  dir="ltr"
                  className="w-full rounded-[20px] border border-[#d9cabb] bg-white px-4 py-3.5 pr-11 text-left text-[#1e1712] outline-none placeholder:text-[#9d8b79]"
            ...
            <div>
              ...
              <div className="relative">
                <input
                  className="w-full rounded-[20px] border border-[#d9cabb] bg-white px-4 py-3.5 pl-20 pr-11 text-right text-[#1e1712] outline-none placeholder:text-[#9d8b79]"
                ...
                <button type="button" onClick={() => setShowPassword((value) => !value)} className="absolute inset-y-0 left-3 inline-flex items-center rounded-xl px-2 text-xs font-semibold text-[#6b5a4c]">
                  {showPassword ? 'إخفاء' : 'إظهار'}
                </button>
              </div>
            </div>
            <button type="submit" disabled={busy} className="inline-flex w-full items-center justify-center gap-2 rounded-[20px] bg-[#1e1712] px-4 py-3.5 text-sm font-semibold text-white shadow-[0_16px_30px_rgba(30,23,18,0.18)] transition hover:translate-y-[-1px] disabled:opacity-60">
              <AppIcon name="crown" className="h-4 w-4 text-[#f1e1cb]" />
              {busy ? 'جارٍ تسجيل الدخول...' : 'دخول إلى الإدارة'}
            </button>
            <button type="button" onClick={() => router.push(resolvedSlug ? `/owner-password?slug=${encodeURIComponent(resolvedSlug)}` : '/owner-password')} className="inline-flex w-full items-center justify-center gap-2 rounded-[20px] border border-[#d9cabb] bg-[#f7efe4] px-4 py-3.5 text-sm font-medium text-[#6b5a4c] transition hover:bg-[#f3e7d7]">
              <AppIcon name="spark" className="h-4 w-4 text-[#9b6b2e]" />
              لدي كود تفعيل أو إعادة تعيين
            </button>
            {err ? (
              <div className="rounded-[20px] border border-[#e6c7c2] bg-[#fff3f1] p-3 text-right text-sm text-[#9a3e35]">
                {err === 'BAD_CREDENTIALS'
