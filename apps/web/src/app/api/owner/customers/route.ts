...
import { createCustomerProfile, listCustomerProfiles } from '@/lib/ops/owner-admin';
...
export async function POST(request: Request) {
  ...
  if (!fullName || !normalizedName || !phoneRaw || phoneNormalized.length < 7) {
  ...
  try {
    ...
    const items = await listCustomerProfiles({ cafeId: ctx.cafeId, databaseKey: ctx.databaseKey }, true);
    const customer = items.find((item) => item.id === customerId) ?? null;
