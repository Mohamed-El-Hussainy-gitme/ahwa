...
export default function OrdersPage() {
  ...
  const filteredProducts = useMemo(
    () => (catalogData?.products ?? []).filter((product) => !effectiveSelectedSectionId || product.sectionId === effectiveSelectedSectionId),
    [catalogData?.products, effectiveSelectedSectionId],
  );
  const addonOptionsByProductId = useMemo(() => {
    const addonMap = new Map((catalogData?.addons ?? []).map((addon) => [addon.id, addon]));
    const map = new Map<string, ProductAddonOption[]>();
    for (const link of catalogData?.productAddonLinks ?? []) {
    ...
    for (const options of map.values()) {
      options.sort((left, right) => left.name.localeCompare(right.name, 'ar'));
    }
    return map;
  }, [catalogData?.addons, catalogData?.productAddonLinks]);
  const productsWithAddons = useMemo(() => new Set((catalogData?.productAddonLinks ?? []).map((link) => link.productId)), [catalogData?.productAddonLinks]);
  const activeAddonProduct = filteredProducts.find((product) => product.id === addonPickerProductId)
    ?? (catalogData?.products ?? []).find((product) => product.id === addonPickerProductId)
    ?? null;
  const activeAddonOptions = activeAddonProduct ? addonOptionsByProductId.get(activeAddonProduct.id) ?? [] : [];
    ...
    async () => {
      ...
      if (creatingNew || !effectiveSessionId) {
        const created = await opsClient.openAndCreateOrder({
          ...
          items: nextDraftLines.map(([productId, quantity]) => ({ productId, quantity, notes: orderNotes || undefined, addonIds: selectedAddons[productId] ?? [] })),
        });
  ...
  function openAddonPicker(productId: string) {
    if (composerOpen) return;
    if (!creatingNew && !effectiveSessionId) {
      warnSessionRequired();
      return;
    }
    setAddonPickerProductId(productId);
  }
    ...
    >
            ...
            {filteredProducts.map((product) => (
              <div key={product.id} className={[opsInset, 'p-3'].join(' ')}>
                <div className="flex items-start justify-between gap-2 text-right">
                  ...
                  {productsWithAddons.has(product.id) ? (
                    <button
                      type="button"
                      onClick={() => openAddonPicker(product.id)}
                      className="shrink-0 rounded-[12px] border border-[#d8c7b3] bg-white px-2.5 py-1 text-[11px] font-semibold text-[#5e4d3f]"
                    >
                      إضافات
                    </button>
                  ) : null}
                ...
                {productsWithAddons.has(product.id) ? (
                  <div className="mt-2 flex items-center justify-end gap-2">
                    {(selectedAddons[product.id]?.length ?? 0) > 0 ? (
                      <div className="text-xs font-semibold text-[#9b6b2e]">+{selectedAddons[product.id]!.length} إضافة</div>
                    ) : null}
                  </div>
                ) : null}
              </div>
      ...
      {composerOpen ? (
        <div className="fixed inset-0 z-[70] flex items-end justify-center bg-[#1e1712]/45 p-3 sm:items-center">
          <div className="w-full max-w-md xl:max-w-[min(40rem,calc(100vw-2rem))] rounded-[28px] border border-[#dccbb7] bg-[#fffdf9] p-4 shadow-[0_24px_60px_rgba(30,23,18,0.22)]">
      ...
      <ProductAddonPicker
        open={Boolean(activeAddonProduct)}
        title={activeAddonProduct?.name ?? ''}
        options={activeAddonOptions}
        selectedIds={activeAddonProduct ? (selectedAddons[activeAddonProduct.id] ?? []) : []}
        onClose={() => setAddonPickerProductId(null)}
        onSave={(nextIds) => {
          if (activeAddonProduct) {
            setSelectedAddons((current) => ({ ...current, [activeAddonProduct.id]: nextIds }));
          }
