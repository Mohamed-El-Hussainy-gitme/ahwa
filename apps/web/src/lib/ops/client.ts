...
export const opsClient = {
  ...
  createOrderWithItems: (input: { serviceSessionId: string; notes?: string; items: Array<{ productId: string; quantity: number; notes?: string; addonIds?: string[] }>; }) => mutate(post<{ ok: true; orderId: string }>('/api/ops/orders/create-with-items', input)),
  ...
  defer: (debtorName: string, allocations: Array<{ orderItemId: string; quantity: number }>, customerId?: string | null) => mutate(post<{ ok: true; paymentId: string; receiptUrl: string; totals: BillingTotals }>('/api/ops/billing/defer', { debtorName, allocations, customerId }, { idempotency: { scope: 'ops.billing.defer' } })),
  deferAndClose: (debtorName: string, allocations: Array<{ orderItemId: string; quantity: number }>, customerId?: string | null) => mutate(post<{ ok: true; sessionClosed: boolean; paymentId: string; receiptUrl: string; totals: BillingTotals }>('/api/ops/billing/defer-and-close', { debtorName, allocations, customerId }, { idempotency: { scope: 'ops.billing.defer-and-close' } })),
  repay: (debtorName: string, amount: number, notes?: string, customerId?: string | null) => mutate(post<{ ok: true }>('/api/ops/deferred/repay', { debtorName, amount, notes, customerId }, { idempotency: { scope: 'ops.deferred.repay' } }), { invalidate: true }),
  addDeferredDebt: (debtorName: string, amount: number, notes?: string, customerId?: string | null) => mutate(post<{ ok: true }>('/api/ops/deferred/add-debt', { debtorName, amount, notes, customerId }, { idempotency: { scope: 'ops.deferred.add-debt' } }), { invalidate: true }),
  ...
  createMenuAddon: (input: { addonName: string; stationCode: StationCode; unitPrice: number; productIds: string[]; }) => mutate(post<{ addonId: string }>('/api/ops/menu/addons/create', input), { invalidate: true }),
  updateMenuAddon: (input: { addonId: string; addonName: string; stationCode: StationCode; unitPrice: number; productIds: string[]; }) => mutate(post<{ ok: true }>('/api/ops/menu/addons/update', input), { invalidate: true }),
  toggleMenuAddon: (addonId: string, isActive: boolean) => mutate(post<{ ok: true }>('/api/ops/menu/addons/toggle', { addonId, isActive }), { invalidate: true }),
  deleteMenuAddon: (addonId: string) => mutate(post<{ ok: true; mode: 'deleted' | 'archived' }>('/api/ops/menu/addons/delete', { addonId }), { invalidate: true }),
};
