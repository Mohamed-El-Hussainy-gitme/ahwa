...
export const opsClient = {
  ...
  openAndCreateOrder: (input: { label?: string; items: Array<{ productId: string; quantity: number }>; }) => mutate(post<{ ok: true; orderId: string; sessionId: string; label: string }>('/api/ops/orders/open-and-create', input)),
  createOrderWithItems: (input: { serviceSessionId: string; items: Array<{ productId: string; quantity: number }>; }) => mutate(post<{ ok: true; orderId: string }>('/api/ops/orders/create-with-items', input)),
