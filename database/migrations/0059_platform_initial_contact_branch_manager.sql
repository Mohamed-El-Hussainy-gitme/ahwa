...
create or replace function public.platform_create_cafe_with_owner(
  ...
  p_owner_label text default 'owner'
)
returns jsonb
language plpgsql
security definer
...
declare
  ...
  v_setup jsonb;
  v_owner_label text := lower(coalesce(nullif(btrim(p_owner_label), ''), 'owner'));
begin
  select *
  into v_admin
  ...
  if v_owner_label not in ('owner', 'partner', 'branch_manager') then
  ...
  insert into ops.cafes (slug, display_name, is_active)
  values (v_slug, btrim(p_cafe_display_name), true)
  ...
  if v_should_create_subscription then
    insert into platform.cafe_subscriptions (
      cafe_id, starts_at, ends_at, grace_days, status, amount_paid, is_complimentary, notes,
      created_by_super_admin_user_id, created_at, updated_at
    )
    values (
      v_cafe_id, p_subscription_starts_at, p_subscription_ends_at, coalesce(p_subscription_grace_days, 0),
      v_subscription_status, coalesce(p_subscription_amount_paid, 0), coalesce(p_subscription_is_complimentary, false),
      nullif(btrim(coalesce(p_subscription_notes, '')), ''), p_super_admin_user_id, now(), now()
    )
    returning id into v_subscription_id;
  end if;

  insert into ops.audit_events (
    cafe_id, actor_type, actor_label, event_code, entity_type, entity_id, payload
  )
  values (
    v_cafe_id,
    'super_admin',
