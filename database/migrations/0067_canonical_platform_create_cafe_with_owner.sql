...
-- Resolve RPC ambiguity and make the create-cafe flow canonical again.
-- We keep a single public RPC that:
-- 1) supports owner / partner / branch_manager labels
-- 2) always issues a one-time password setup code
-- 3) avoids ambiguous PostgREST overload resolution
...
begin
  ...
  -- kept for backward RPC compatibility; password is created through one-time setup
